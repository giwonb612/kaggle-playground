...
def main():
    ...
    print("=" * 55)
    print("v13 유망 후보 10-seed 검증")
    print("기준: v10 10-seed 84.12%")
    print("=" * 55)

    results = {}
    for name, feat_set in CANDIDATES.items():
        total_feats = len(feat_set["num"]) + len(feat_set["cat"]) + 2
        print(f"\n--- {name} ({total_feats}개 피처) ---")
        sys.stdout.flush()
        ...
        for seed in SEEDS_FULL:
            ...
            print(f"  seed={seed:3d} | CV={cv_scores.mean():.4f}")
            sys.stdout.flush()
        ...
        print(f"  → 10-seed: {avg_cv:.4f} ±{std_cv:.4f}  생존:{n_survived}  diff={diff:+.4f}")
        results[name] = (avg_cv, std_cv, n_survived, np.mean(test_probas, axis=0))
        sys.stdout.flush()

        if avg_cv > 0.8412:
            pd.DataFrame({"PassengerId": test_ids, "Survived": avg_preds}).to_csv(
                SUBMISSIONS / f"submission_v13_{name.lower()}_10s.csv", index=False)
            print(f"  → 저장: submission_v13_{name.lower()}_10s.csv")
            sys.stdout.flush()

    # 최종 요약
    print(f"\n{'='*55}")
    print("최종 요약")
    print(f"{'실험':<15} | {'10-seed CV':>10} | {'분산':>8} | {'diff':>8}")
    print("-" * 55)
    for name, (cv, std, ns, _) in results.items():
        diff = cv - 0.8412
        star = " ★" if cv > 0.8412 else ""
        print(f"{name:<15} | {cv:.4f}     | ±{std:.4f} | {diff:+.4f}{star}")
    sys.stdout.flush()
